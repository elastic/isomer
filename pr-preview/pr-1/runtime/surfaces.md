---
title: Surfaces
description: A surface is one output format. Every surface takes the same Composition and exposes the same two methods — render for a whole composition, renderNode...
url: https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/surfaces
---
...
`HTMLRenderResult` is `{ html, css, body, measurement, validationErrors }`. [Embedding](https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/embedding) covers getting that pair onto a page a host already controls. The `svg` entry is `undefined` unless the runtime was given [frames](https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/frame), and the factory's return type tracks which.
...
The first parameter is the host context views build with, the second the render context, the third the theme; [Runtime](/pr-preview/pr-1/runtime/runtime#typing-the-render-context) covers when each is worth naming.
...
[Frame](https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/frame) · [Runtime](https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/runtime) · [API reference](https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/api)