...
description: The authoring context is what a host hands an agent so it can answer a question with a composition: the composition contract, what it may compose from,...
url: https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/authoring-context
---
...
Pass `authoring` on `createIsomerRuntime` to name pack-owned `$defs` (`actionItem`, `badgeItem`), attach refine descriptions, or hide a legacy alias. The validator schema `parse` uses is unchanged. A pack can contribute its own `describe`/`omitProperties` too, via `authoring` on its `PrimitivePackInput` — see [Packs](/pr-preview/pr-1/sdk/packs#authoring) — and the runtime merges every composed pack's contribution with this option, which wins on conflict.
...
`formats` reports the surfaces that were actually built — a runtime with no [frame](https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/frame) truthfully omits `svg`.

## Next

[View registry](https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/view-registry) for the trusted path · [Surfaces](https://docs-v3-preview.elastic.dev/pr-preview/pr-1/runtime/surfaces) for rendering what comes back