...
Registering the same id twice throws. Registering after construction is expected, not a workaround: a host may build views per request or load them from a store, and [the authoring context](authoring-context.md) reads the list live for exactly that reason.
...
[Authoring context](authoring-context.md) for the other path into a composition · [Surfaces](surfaces.md) for what happens after
